...
dataManagerPath = os.path.dirname(os.path.realpath(__file__))
...
class DataManager(object):
  ...
  conn = {}
  settings = {}
  settingFilePath = dataManagerPath+'/DataManager.json'
  if os.path.isfile( settingFilePath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,settingFilePath) ) == True:
    settingFilePath = "%s/%s"%(dataManagerPath,settingFilePath)
  ...
  def csvToDb():
    ...
    hobbyPath = DataManager.setting('hobbyCardCsvPath')
    if os.path.isfile( hobbyPath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,hobbyPath) ) == True:
      hobbyPath = "%s/%s"%(dataManagerPath,hobbyPath)
    f = open( hobbyPath )
    ...
    skillColumnMap = {}
    needColumns = []
    playerPath = DataManager.setting('playerCardCsvPath')
    if os.path.isfile( playerPath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,playerPath) ) == True:
      playerPath = "%s/%s"%(dataManagerPath,playerPath)
    f = open(playerPath)
    ...
    skillColumns = []
    needColumns = []
    childPath = DataManager.setting('childCardCsvPath')
    if os.path.isfile( childPath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,childPath) ) == True:
      childPath = "%s/%s"%(dataManagerPath,childPath)
    f = open(childPath)
    ...
    needColumns = []
    partnerPath = DataManager.setting('partnerCardCsvPath')
    if os.path.isfile( partnerPath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,partnerPath) ) == True:
      partnerPath = "%s/%s"%(dataManagerPath,partnerPath)
    f = open(partnerPath)
    ...
    skillRequirementColumns = []
    needColumns = []
    jobPath = DataManager.setting('jobCardCsvPath')
    if os.path.isfile( jobPath ) == False and os.path.isfile( "%s/%s"%(dataManagerPath,jobPath) ) == True:
      jobPath = "%s/%s"%(dataManagerPath,jobPath)
    f = open(jobPath)
  ...
  def createGameDb():
    ...
    dbPath = DataManager.setting('gameResultsDbPath')
    dirCheck = re.sub( r'[\\\/][^\\\/]*$', r'', dbPath )
    if os.path.isdir( dirCheck ) == False and os.path.isdir( "%s/%s"%(dataManagerPath,dirCheck) ) == True:
      dbPath = "%s/%s"%(dataManagerPath,dbPath)

    if os.path.isfile( dbPath ) == False:
      copyfile( \
          dataManagerPath+"/"+DataManager.setting("dbPath"), \
          DataManager.setting("gameResultsDbPath") )
